...
- (void)replaceImageText
{
    ...
    for (NSString *imageUrl in [self imagesOfRegexStrArray]) {
        ...
        int location = (int )([lastValue rangeValue].location + ([lastValue rangeValue].length == 0 ? 0 : 1));
        //获取当前字符串在文本中的位置
...
- (NSArray *)rangesOfContents:(NSArray *)contents
{
    ...
    for (NSString *content in contents) {
        ...
        int location = (int )([lastValue rangeValue].location + [lastValue rangeValue].length);
        //获取当前字符串在文本中的位置
